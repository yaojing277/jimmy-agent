...
- 所有由 Codex 建立的檔案，**一律存放於 `/Users/jimmy/Downloads/jimmy-agent/jimmy_scripts/`**
...
| `update_wealth_os.py` | 「**同步股價**」（＝更新 Wealth OS；注意與「更新股價」不同）：把最新 `Jimmy_YYMMDD` 分頁同步到雲端 `Jimmy_Wealth_OS_Master_V4.4_Google.xlsm` 的「02_持股總表」（Drive API 就地覆蓋、檔案 ID 不變）；`update`/`--yes`/`--dry-run`；需 token 含 Drive 權限，細節見 memory |
...
| YouTube 影片 AI 摘要 | `yt_summary.py <網址>`（本機工具，不部署）：字幕（youtube-transcript-api，zh-TW 優先）→ Codex CLI `-p`（自動尋找桌面版 App 內建執行檔）→ 深色 HTML 摘要頁＋`yt_summaries/index.html` 摘要庫；無字幕影片直接跳過；**首次使用需先讓 Codex CLI /login 一次** |
...
- [2026-07-12] **YouTube 影片 AI 摘要工具**（v1.0 完成，已實測可用）
  - `jimmy_scripts/yt_summary.py`：貼網址 → 抓字幕 → Codex CLI 摘要 → HTML 摘要頁＋摘要庫 index；已加入 projects.html 主要專案卡片
  - 已驗證：網址解析（watch/youtu.be/shorts/live）、字幕抓取與語言優先序、時間戳章節跳轉連結、無字幕優雅跳過（卡哇KAWA 頻道全片關閉字幕，無法摘要該頻道）
  - Codex CLI 已完成首次 `/login`，真實摘要實測成功（TED 拖延症演講、阿格力 EP134）
  - 後續延伸（未做）：接上 youtube_notify.py 自動摘要新片、摘要庫上 GitHub Pages
