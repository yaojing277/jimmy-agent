...
console.log('[AudioTab] background.js loaded');

const removeTimers = {};

browser.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  console.log('[AudioTab] onUpdated', tabId, JSON.stringify(changeInfo));
  if (changeInfo.audible === true) {
    clearTimeout(removeTimers[tabId]);
    injectHighlight(tabId, true);
  } else if (changeInfo.audible === false) {
    clearTimeout(removeTimers[tabId]);
    removeTimers[tabId] = setTimeout(() => injectHighlight(tabId, false), 2000);
  }
});

browser.runtime.onStartup.addListener(() => {
  console.log('[AudioTab] onStartup');
  scanAllTabs();
});
browser.runtime.onInstalled.addListener(() => {
  console.log('[AudioTab] onInstalled');
  scanAllTabs();
});

function scanAllTabs() {
  browser.tabs.query({ audible: true }).then((tabs) => {
    console.log('[AudioTab] scanAllTabs found', tabs.length, 'audible tabs');
    tabs.forEach((tab) => injectHighlight(tab.id, true));
  });
}

function injectHighlight(tabId, isPlaying) {
  console.log('[AudioTab] injectHighlight', tabId, isPlaying);
  browser.scripting.executeScript({
    target: { tabId },
    func: isPlaying ? applyAudioHighlight : removeAudioHighlight,
  }).catch((e) => console.error('[AudioTab] executeScript error:', e));
}
