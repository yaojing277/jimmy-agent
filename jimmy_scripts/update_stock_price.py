...
# -*- coding: utf-8 -*-
"""
update_stock_price.py —「更新股價」用副本(由 update_close_price.py 複製)。
依 A 欄代號抓收盤價,寫回指定列的價格欄(預設 K)。

★「更新股價」月度滾動(本檔新增,對應觸發語「更新股價」):
  python3 update_stock_price.py update            # 先預覽再確認
  python3 update_stock_price.py update --yes       # 免確認直接寫
  python3 update_stock_price.py update --dry-run   # 只預覽不寫入
  python3 update_stock_price.py update --date 2026/7/1
  流程:日期欄(H/I/J/K)整體左移一格 → 丟掉最舊 H、H←I、I←J、J←K
        (標頭日期與價格一起搬)→ K1 換成執行月日 → K 欄重抓當日收盤。
  所有公式固定指向最後日期欄(現今價),滾動後不需改任何公式。
  抓不到報價的列(如美股)一律留白,比照全新欄位。
...
# ========================= 認證 =========================
def get_sa_creds():
    """GitHub Actions 等無人環境:環境變數 GOOGLE_SA_JSON = Service Account 金鑰 JSON 內容。
    Service Account 不會過期;試算表與 xlsm 需分享給 SA 的 client_email。"""
...
def get_service():
    sa = get_sa_creds()
    if sa:
        return build("sheets", "v4", credentials=sa)
    creds = None
...
def get_sheet_props(service):
    ...
    for s in meta.get("sheets", []):
        p = s["properties"]
        out[p["title"]] = (p["sheetId"], p["index"])
    return out


def duplicate_sheet_as(service, src_title, new_title, insert_index):
    """複製 src_title 分頁為 new_title(含公式/格式),插在 insert_index。
    new_title 已存在則中止(同天重跑防呆)。回傳新分頁 sheetId。"""
...
# ========================= 指令:update(滾動視窗+更新現今價) =========================
def cmd_update(service, args):
    """月度滾動:日期欄整體左移一格(丟最舊),最右欄換成執行日並重抓當日收盤。
    例:H←I、I←J、J←K(含標頭日期一起搬),K1=執行月日,K 欄重抓今日收盤。
    所有公式固定指向最後日期欄(LAST),滾動後不需改任何公式。

    預設(快照模式):先把最新分頁複製成「Jimmy_當天YYMMDD」新分頁(插於舊分頁左側),
    於新分頁上滾動,舊分頁保留為凍結快照。--in-place 則就地滾動最新分頁(舊行為)。"""
    global SHEET_NAME
    exec_date = parse_date(args.date) if args.date else datetime.date.today()
    if args.date and not exec_date:
        sys.exit(f"無法解析日期:{args.date}(請用 2026/7/1 格式)")

    # 快照模式(預設):複製最新分頁 → 命名為當天日期 → 於副本上滾動
    if not args.in_place:
        ...
        if args.dry_run:
            ...
            src_index = props[src_title][1]
            duplicate_sheet_as(service, src_title, new_title, src_index)
            print(f"[snapshot] 已複製 {src_title} → {new_title}(插於其左側);"
                  f"於新分頁滾動,舊分頁保留為快照。\n")
            SHEET_NAME = new_title

    cols, date_cols, LAST = get_layout(service)
    if len(date_cols) < 2:
        sys.exit(f"日期欄少於兩欄({[c for c, _ in date_cols]}),無法滾動。")
    letters = [c for c, _ in date_cols]
    first, last = letters[0], letters[-1]          # 例 H, K(last == LAST)
    date_of = dict(date_cols)

    # 找最後資料列(A 欄最後一個非空列)
    acol = service.spreadsheets().values().get(
        spreadsheetId=SPREADSHEET_ID,
        range=f"{SHEET_NAME}!A{DATA_START_ROW}:A{args.max_row}").execute().get("values", [])
    last_row = DATA_START_ROW - 1
    for i, row in enumerate(acol, start=DATA_START_ROW):
        if row and str(row[0]).strip() != "":
            last_row = i
    if last_row < DATA_START_ROW:
        sys.exit("A 欄找不到任何資料列。")

    def av(r):
        idx = r - DATA_START_ROW
        return str(acol[idx][0]).strip() if 0 <= idx < len(acol) and acol[idx] else ""

    # 讀日期欄整塊(標頭列 1 ~ last_row);UNFORMATTED 保留日期序號與數值
    block = service.spreadsheets().values().get(
        spreadsheetId=SPREADSHEET_ID,
        range=f"{SHEET_NAME}!{first}1:{last}{last_row}",
        valueRenderOption="UNFORMATTED_VALUE").execute().get("values", [])
    base = letter_to_idx(first)
    def bv(r, letter):
        i, j = r - 1, letter_to_idx(letter) - base
        if 0 <= i < len(block) and 0 <= j < len(block[i]):
            return block[i][j]
        return ""

    # 逐列抓執行日收盤(僅台股;抓不到者留白)
    print(f"分頁「{SHEET_NAME}」滾動更新;現今價欄={LAST},執行日={exec_date}\n")
    dropped = date_of.get(first)
    last_date = date_of.get(last)
    if isinstance(last_date, datetime.date) and exec_date <= last_date:
        print(f"⚠ 執行日 {exec_date} 不晚於現有最右日期 {last_date};"
              f"滾動會丟掉 {first}({dropped}) 且新舊日期可能重複,請確認。\n")

    print("日期欄滾動(標頭+價格整欄左移,丟棄最舊):")
    for dst, src in zip(letters[:-1], letters[1:]):
        print(f"  {dst}: {date_of.get(dst)} ← {src}({date_of.get(src)})")
    print(f"  {last}(現今價): {last_date} → {exec_date}(重抓收盤)")
    print(f"  丟棄最舊:{first} 的 {dropped}\n")

    kprices = {}   # row -> (a, used_symbol, price|None)
    for r in range(DATA_START_ROW, last_row + 1):
        ...
        price, used = None, ""
        for sym in resolve_symbol(a):
            p, _cur = fetch_close(sym, exec_date)
            used = sym
            if p is not None:
                price = p
                break
        kprices[r] = (a, used, price)

    print(f"{last} 欄重抓 {exec_date} 收盤:")
    miss = []
    for r in range(DATA_START_ROW, last_row + 1):
        if r not in kprices:
            continue
        a, used, price = kprices[r]
        if price is not None:
            print(f"  {last}{r}  A={a!r:>10} [{used}] = {price}")
        else:
            print(f"  {last}{r}  A={a!r:>10} [{used or '?'}] = (抓不到,留白)")
            miss.append(r)

    ok = sum(1 for r in kprices if kprices[r][2] is not None)
    if args.dry_run:
        print("\n[dry-run] 未寫入。")
        return
    if not args.yes:
        ans = input(f"\n確認滾動 {first}:{last} 並更新 {last} 欄"
                    f"({ok} 檔有報價,{len(miss)} 檔留白)?(yes/no): ").strip().lower()
        if ans != "yes":
            print("已取消。")
            return

    data = []
    # 1) 左移:dst 欄整欄(標頭列 1 ~ last_row)取 src 欄值
    for dst, src in zip(letters[:-1], letters[1:]):
        colvals = [[bv(r, src)] for r in range(1, last_row + 1)]
        data.append({"range": f"{SHEET_NAME}!{dst}1:{dst}{last_row}", "values": colvals})
    # 2) 最右欄標頭 = 執行月日
    data.append({"range": f"{SHEET_NAME}!{last}1",
                 "values": [[f"{exec_date.year}/{exec_date.month}/{exec_date.day}"]]})
    # 3) 最右欄資料:重抓收盤;抓不到或非資料列一律留白(比照全新欄位)
    kcol = [[kprices[r][2] if (r in kprices and kprices[r][2] is not None) else ""]
            for r in range(DATA_START_ROW, last_row + 1)]
    data.append({"range": f"{SHEET_NAME}!{last}{DATA_START_ROW}:{last}{last_row}", "values": kcol})
    ...
    print(f"\n完成,更新 {resp.get('totalUpdatedCells')} 格。")
    if miss:
        print(f"留白(抓不到報價,需手動或補 ALIAS)的列:{','.join(map(str, miss))}")
...
def main():
    ...
    p_up = sub.add_parser("update", help="快照滾動:複製最新分頁為當天日期→於副本滾動(--in-place 則就地)")
